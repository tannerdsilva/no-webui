...
// known-answer tests from rfc 4231 — the same vectors run on macOS and linux,
// proving the pure-swift hmac-sha256 is byte-identical (and correct) on both.
...
struct RFC4231Tests {
	...
	func case1() {
		...
		#expect(HMACSHA256.hex(message: message, key: key) == expected)
	}
	...
	func case5() {
		...
		let full = HMACSHA256.hex(message: message, key: key)
	...
	@Test("sha-256 empty-input digest is the canonical null hash")
	func emptyMessageDigest() {
		let hash = HMACSHA256.authenticate(message: [], with: [])
		#expect(bytesToHex(hash) == "b613679a0814d9ec772f95d778c35fc5ff1697c493715653c6c712144292c5ad")
	}

	@Test("string-based hex API matches byte-based API")
	func stringAPI() {
		let viaBytes = HMACSHA256.hex(message: Array("message".utf8), key: Array("key".utf8))
		let viaStrings = HMACSHA256.hex(message: "message", key: "key")
