...
the styled components (`WebUIButton` … `WebUITable` …) and the theme layer
(`WebUITheme`, `ColorScheme`, `WebUIThemeProvider`, and the generated
`DesignToken` vocabulary) live in the wasm-clean `WebUIDesignSystemCore`
target (dependencies: `WebUICore`, `Logging`). `DesignToken` is emitted there
at build time by `WebUIAssetPlugin` as `DesignTokens+Generated.swift`;
`WebUIDesignSystem` re-exports the target (`@_exported import`) and keeps the
`@Theme` macro, `WebUIDocument`, and `DesignSystemAssets` (the server-bound
shim over `WebUIAssets`). components and the theme layer compile into the wasm
client unchanged.
...
`DesignToken` (generated by `WebUIAssetPlugin` into `WebUIDesignSystemCore`
from this stylesheet) is the typed vocabulary of those 169 tokens:
`DesignToken.colorPrimarySolid` → `--color-primary-solid`. component-scoped
custom properties — `.btn { --btn-bg: … }` and friends — are deliberately not
part of `DesignToken`: they are scoped to their selector and would ignore a
`:root` override. restyle those through a theme's `rules` instead.
