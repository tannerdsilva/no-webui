...
| Large-response truncation | the reference servers set an explicit 8 MB `SO_SNDBUF`: a `Connection: close` response whose tail did not fit the default send buffer was silently truncated (probe-verified; the `NIOAsyncChannel` write is not promise-awaited) |
