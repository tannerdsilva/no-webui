...
| Memory ceiling on small hosts | `ConnectionGate` caps concurrent connections (`--max-connections`, default 256) and a 120 s read-idle reaper guards plain http and websockets alike — bounded page-sized buffers and no silent sockets on a 2 gb box (`--event-loops` sizes the nio group) |